...
public class ListaEnlazadaGenerica<T> extends ListaGenerica<T> {
	...
	public boolean agregar(T[] e){
		boolean state = false;
		int prevSize = this.tamanio;
		for (int i = 0; i < e.length; i++) {
			this.agregarFinal(e[i]);
		}
		state = (this.tamanio == (prevSize + e.length));
		// NO debería haber nunca algún problema con esto, siempre seria true a menos que se rompa
		// En ese caso ni debería devolver algo
		return state;
	}
